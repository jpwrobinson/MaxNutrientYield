...
# drawnow
  ...
  # Available food:
  phiprey <-  t(dwPP*wPP*nPP) %*% t(predkernelPP[1:nGrid,]) + 
  ...
  if (length(param$h) == 1){
    ...
    for (jSpecies in 1:nSpecies){
    f[jSpecies,] <- 1-
      ( t(IntakeMax[jSpecies,]/SearchVol[jSpecies,]/
            (phiprey+(IntakeMax[jSpecies,]/SearchVol[jSpecies,])) ))
...
Rp <- 0.5*param$rho[i]*Egg/w[1] + param$R0[i]; # Egg flux (numbers/time)
